...
import ec.edu.espe.zonas.exceptions.OperacionNoPermitidaException;
...
public class ZonaServicioImpl implements ZonaServicio {
    ...
    @Override
    public ZonaResponseDto crearZona(ZonaRequestDto requestDto, String ip, String usuario, String rol) {
        ...
        auditEventPublisher.publicar("CREATE", "ZONA", dto, ip, usuario, rol);
        return dto;
    }

    @Override
    public ZonaResponseDto actualizarZona(UUID idZona, ZonaRequestDto request, String ip, String usuario, String rol) {
        // Validación adicional: La capacidad no puede ser negativa
        ...
        auditEventPublisher.publicar("UPDATE", "ZONA", dto, ip, usuario, rol);
        return dto;
    }

    @Override
    public void activarDesactivar(UUID idZona, String ip, String usuario, String rol) {
        Zona zonaExistente = zonaRepository.findById(idZona)
                .orElseThrow(() -> new RecursoNoEncontradoException("Zona no encontrada con ID: " + idZona));
        ...
        Zona zonaActualizada = zonaRepository.save(zonaExistente);
        auditEventPublisher.publicar("UPDATE", "ZONA", convertirADto(zonaActualizada), ip, usuario, rol);
    }
