...
export class UserRoleService {
  ...
  async assignRole(
    ...
    if (existe) {
      ...
      await this.emitEvent('CREATE', saved, requesterUsername, ip, requesterRoles[0]);
      return saved;
    }
  ...
  async setActive(
    ...
    const saved = await this.userRoleRepository.save(userRole);
    await this.emitEvent('UPDATE', saved, requesterUsername, ip, requesterRoles[0]);
    return saved;
  }
  ...
  async removeRole(
    ...
    await this.emitEvent('DELETE', existe, requesterUsername, ip, requesterRoles[0]);
  }
